...
if(isset($_POST['submit'])) {
	...
	// set the name of the new category php file that will be generated by the function newCategory_include() found within functions.php. 
	// strtolower() is a php function that will ensure that the file name is all lowercase characters. 
	$filename = strtolower($filename); 
	$description = mysqli_real_escape_string($dbcon, $_POST['description']);
	$display = mysqli_real_escape_string($dbcon, $_POST['header_display']); // sets if the url to the new category will be displayed in the navigation header or not
	...
	if(!mysqli_stmt_prepare($stmt, $sql)){
		...
		*/
		...
		{
			$handle = fopen($_SERVER['DOCUMENT_ROOT'].'/'.$my_file, 'w') or die('Cannot open file:  '.$my_file);
		}

		// this $type variable will be used to pass the category name to the display_artwork() function found within functions.php. 
		// display_artwork($type) is called from within the new category file created by the newCategory_include function, which generated a new php file for the new category.
		$type='$type';

		/* 
		Creates the data for a new category php file. The name of the php file will match the name of the category.
		This is done so that the link to the category found in the header bar will match the name of the category.
		(a) admin creates a category called 'photography'
		(b) a file named 'photography.php' is generated
		(c) the header bar now contains a url linking to the photography.php file, 
		    so the url would read 'https://localhost/artist-portfolio/photography'
		*/
		$function_data = newCategory_include($filename,$type,$category_name);
		$data = $function_data;

		// generate the php file itself
		fwrite($handle, $data);
	}
}
else {
