...
$sql = "SELECT COUNT(*) FROM category_artwork";
...
<?php // The following are headers for the top of the table that will display all of the art category information to the admin. ?>
...
<?php
// This php block will store the art category information returned from the database query in variables and display the table to the user.

// A select statement  to return a result-set of art category data based on where the administrator is in the pagination navigation
$sql = "SELECT * FROM category_artwork ORDER BY id DESC LIMIT $offset, $rowsperpage";
// executes a prepared query and stores the result as a result set or FALSE
$result = mysqli_query($dbcon, $sql);

// if no art category data is returned from the query, then let the user know
if(mysqli_num_rows($result) < 1) {
	echo "No category found";
} 

// Store the art category information returned from the database query in variables and .
